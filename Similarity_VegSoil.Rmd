...
species.wide2 <- species.wide[-c(1:3)]
species.wide2 <- as.matrix(species.wide2)
head(species.wide2)

vegdist.out <- vegdist(species.wide2, method="bray", binary=TRUE)
matrix.out <- as.matrix(vegdist.out)
view(matrix.out)

species.wide$UniqueID <- paste(species.wide$Transect, species.wide$VegSoil, species.wide$Bank_Location, sep = "-") #pasting columns together

species.wide <- species.wide[,c(101,1:100)] #move uniqueID column to front

species.wide <- species.wide[order(species.wide$UniqueID),] #put in order

species.wide <- as.data.frame(species.wide)

rownames(species.wide) <- species.wide$UniqueID

species.wide2 <- species.wide[-c(1:4)]
...
as.matrix(species.wide2)[df$Spe]
heatmap(as.matrix(vegdist))
