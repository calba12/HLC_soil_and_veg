...
library(knitr); library(multcomp); library(dplyr);library(lme4);library(ggplot2); library(tidyverse); library(ggpubr); library(rstatix);library(writexl);library(ggformula); library(tidyr);library(reshape2); library(sjPlot); library(emmeans)
...
```{r graph WIS by bank location then move on to model}
...
#Proportion upland species; Gaussian family fixed and random effects
lm_gaus_null <- lm(Prop.UPL ~ 1, data = WIS_bankLoc)
lm_gaus_UPL <- lm(Prop.UPL ~ Location, data = WIS_bankLoc) 

AIC(lm_gaus_null,lm_gaus_UPL)

glm_gaus_UPL <- lmer(Prop.UPL ~ Location + (1|Baseline),data = WIS_bankLoc)

effectsize::standardize(glm_gaus_UPL) %>% summary()
plot_residuals(glm_gaus_UPL)
plot_model(glm_gaus_UPL, type = "diag")
sjPlot::plot_model(glm_gaus_UPL , type = "est", show.values = T)

sjPlot::tab_model(glm_gaus_UPL, title = "Upland WIS", dv.labels = "Proportion Cover")

emmeans(glm_gaus_UPL, pairwise ~ Location, adjust = "tukey")
...
FG_bankLoc <- select(FG_bankLoc, -Total.Hits, -Hit.by.loc, -PG, -PF, -AF, -VF, -BF, -All.SLF, -W, -AG, -prop.W, -prop.AG)
...
p <- FG_bankLoc_long %>% ggplot(aes(x=variable, y=value))+
  geom_boxplot()+
  geom_point()+
...
p <- df_FG_baseline_means %>% ggplot(aes(x=variable, y=Mean))+
  geom_boxplot()+
  geom_point()+
