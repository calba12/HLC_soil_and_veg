...
p <- df_prop_hit_type_means %>% ggplot(aes(x=factor(Surface.Hit, levels = c('litter', 'Vegetation', 'bare soil', 'downed woody debris', 'water')), y=Mean))+
  geom_boxplot()+
  geom_point()+
  labs(x = "Cover Type", y = "Proportion of Cover")+
  stat_summary(fun=mean, geom="point", aes(group = Bank_Location), position = position_dodge(.9), shape=20, size=2.5, color="red", fill="red")+
  theme_bw(base_size = 16)+
  theme(axis.text.x=element_text(angle=90, hjust=1)) +
...
lmer_gaus1 <- lmer(prop ~ WIS_GP + (1|baseline) + (1|Transect:baseline), data = df.props_WIS_veg) 
lmer_gaus2 <- lmer(prop ~ WIS_GP + Bank_Location + (1|baseline) + (1|Transect:baseline), data = df.props_WIS_veg) 
lmer_gaus3 <- lmer(prop ~ WIS_GP + Bank_Location + WIS_GP*Bank_Location + (1|baseline) + (1|Transect:baseline), data = df.props_WIS_veg)
lmer_gaus4 <- lmer(prop ~ WIS_GP + Bank_Location + WIS_GP*Bank_Location + (1|baseline), data = df.props_WIS_veg)

AIC(lmer_gaus_null, lmer_gaus1,lmer_gaus2,lmer_gaus3, lmer_gaus4)
...
#Different ways to visualize the distribution of the data
...
emmip(lmer_gaus4, WIS_GP ~ Bank_Location)
...
lmer_gaus_native_1 <- lmer(prop ~ Nativity + (1|baseline) + (1|Transect:baseline), data = df.props_Nativity_veg)
lmer_gaus_native_2 <- lmer(prop ~ Nativity + (1|baseline) + (1|Transect:baseline), data = df.props_Nativity_veg) 
lmer_gaus_native_3 <- lmer(prop ~ Nativity + Bank_Location + Nativity*Bank_Location + (1|Transect:baseline) + (1|Transect:baseline), data = df.props_Nativity_veg)

AIC(lmer_gaus_native_1, lmer_gaus_native_2, lmer_gaus_native_3)

summary(lmer_gaus_native_1)
summary(lmer_gaus_native_2)
summary(lmer_gaus_native_3)

qqnorm(resid(lmer_gaus_native_3)) #Use to visualize whether normally distributed
plot(lmer_gaus_native_3)

proportion_Native_gauss <- df.props_Nativity_veg$prop #Histogram showing lots of zeroes, positive skew 
hist(proportion_Native_gauss)


#Choose lmer_gaus_upper1 based on AIC and run diagnostics, output, pairwise comparisons

#effectsize::standardize(lmer_gaus_upper2) %>% summary()
plot_residuals(lmer_gaus_native_3)
plot_model(lmer_gaus_native_3, type = "diag")
sjPlot::plot_model(lmer_gaus_native_3, type = "est", show.values = T)

sjPlot::tab_model(lmer_gaus_native_3, df.method = "satterthwaite", title = "Proportion Native and Introduced in Vegetation", dv.labels = "Proportion Cover")

emmeans(lmer_gaus_native_3, df.method = "satterthwaite", pairwise ~ Nativity | Bank_Location, adjust = "tukey")
emmip(lmer_gaus_native_3, Nativity~ Bank_Location)
...
AIC(lmer_gaus_fg_1, lmer_gaus_fg_2,lmer_gaus_fg_3)

summary(lmer_gaus_fg_1)
summary(lmer_gaus_fg_2)
summary(lmer_gaus_fg_3)

qqnorm(resid(lmer_gaus_fg_1)) #Use to visualize whether normally distributed
plot(lmer_gaus_fg_1)

proportion_FunctionalGroup_gauss <- df.props_FG$prop #Histogram showing lots of zeroes, positive skew 
hist(proportion_Native_gauss)
...
#effectsize::standardize(lmer_gaus_upper2) %>% summary()
plot_residuals(lmer_gaus_fg_1)
plot_model(lmer_gaus_fg_1, type = "diag")
sjPlot::plot_model(lmer_gaus_fg_1, type = "est", show.values = T)

sjPlot::tab_model(lmer_gaus_fg_1, df.method = "satterthwaite", title = "Proportion of Functional Groups in Vegetation", dv.labels = "Proportion Cover")

emmeans(lmer_gaus_fg_1, df.method = "satterthwaite", pairwise ~ FunctionalGroup, adjust = "tukey")
