...
rename(Num_transect_points = Number.of.points.along.transect_ChrissyCalc) %>%
  ...
  mutate(Bank_Location = recode(Bank_Location, "lower" = "Lower", "middle" = "Middle", "upper" = "Upper"))
...
```{r graphs for hit type by bare soil, litter, vegetation, water, dwd}
...
p <- df_prop_hit_type_means %>% ggplot(aes(x=factor(Surface.Hit, levels = c('Litter', 'Vegetation', 'Bare Soil', 'Woody Debris', 'Water')), y=Mean, fill = Surface.Hit))+
  geom_boxplot()+
  geom_point()+
  labs(x = "Cover Type", y = "Proportion of Cover")+
...
Want to redo common species using proper prepping steps within R but can use this for HLC report due Nov 2022
...
#Get some means for HLCC report
...
p <- df.props_FG_means %>% ggplot(aes(x=factor(FunctionalGroup, levels = c('PG', 'PF', 'W', 'SLF', 'AG')), y=Mean, fill = FunctionalGroup))+
  geom_boxplot()+
  geom_point()+
...
p <- ggplot(df.props_FG_stacked_baseline, aes(x = factor (baseline, level = c("B03", "B05", "B07", "B10", "B13", "B14", "B15", "B17", "B18")), y = Mean, fill = FunctionalGroup))+
  geom_bar(position = "stack", stat = "identity", color = "black") +
  labs(x = "Baseline", y = "Proportion of Cover") +
  guides(fill = guide_legend("Functional Group Type")) +
  theme_bw(base_size = 16) + 
...
#Some additional stats for HLCC report

df.props_FG_means <- df.props_FG %>%
  group_by(baseline, FunctionalGroup) %>%
  dplyr::summarise(Mean = mean(propFG), sd = sd(propFG))
