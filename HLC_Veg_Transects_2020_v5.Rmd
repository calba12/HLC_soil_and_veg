...
```{r hit type bare soil, litter, vegetation}

#Create general categories based on whether vegetation was hit first, or pin dropped straight to bare ground, litter, water
# To do this, if surface hit is NA, then hit type is vegetation, otherwise bare soil or litter as designated in the column
...
#The below is not right yet; need to figure out how to count by veg/litter/bare soil in column Surface.Hit

hit_type$Surface.Hit <- as.factor(hit_type$Surface.Hit)

df.prop_hit_type <- hit_type %>%
  group_by(Hit.Number, Bank_Location, baseline, Transect) %>%
  summarise(Surface.Hit = n()) %>%  ## from the group_by groupings, count the number 
  group_by(baseline, Bank_Location, Surface.Hit) %>%  
  summarise(Surface.Hit = sum(Surface.Hit)) %>% ## groups by baseline and Bank_location to sum Surface.Hit
  mutate(prop = Surface.Hit/sum(Surface.Hit)) %>% ## mutate the df by adding prop that is each #hit type/total
  complete(Surface.Hit, fill = list(Surface.Hit = 0, prop = 0)) ## keep all levels of factor and replace NA with zeros for both Surface.Hit and prop

# Check it adds to one with the 5 groups  
sum(df.prop_hit_type$prop[1:6])
