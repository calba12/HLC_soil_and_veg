...
```{r graph WIS by baseline ---> don't use :) }
...
#Make boxplot --> AI 9/27/22
WIS_veg <- df_WIS_means_veg %>% 
...
WIS_BL_Veg <- WIS_veg + facet_wrap(~Bank_Location)
print(WIS_BL_Veg)
...
#Make boxplot _CA CODE
p <- df.props_WIS_veg_exp %>% ggplot(aes(x=factor(WIS_GP, levels = c('UPL', 'FACU', 'FAC', 'FACW', 'OBL')), y=prop))+ 
  geom_boxplot()+
  geom_point()+
...
#Make stacked bar graph __ CA CODE

df_stacked_WIS <- aggregate(prop ~ Bank_Location + WIS_GP, data = df.props_WIS_veg, FUN = mean)

View(df_stacked)

ggplot(df_stacked_WIS, aes(fill = WIS_GP, y = prop, x = Bank_Location))+
  geom_bar(position = "stack", stat = "identity", color = "black") +
  labs(x = "Bank Location", y = "Proportion of Cover") +
...
```{r graph functional group - AI 09/27/22}
...
df.props_FG_veg <- vegdf %>%
  ...
  summarise(Species.Hit = sum(Species.Hit)) %>% ## groups by baseline and Bank_location to sum species.Hit
  mutate(prop = Species.Hit/sum(Species.Hit)) %>% ## mutate the df by adding prop that is each #sp/total
  complete(FunctionalGroup2, fill = list(Species.Hit = 0, prop = 0)) %>% ## keep all levels of factor and replace NA with zeros for both Species.Hit and prop
...
#this one
Nativity_v <- c("Native", "Introduced")
Short_lived_v <- c(19, 12)
Long_lived_v <- c(216, 96)
OBL_FACW_v <- c(167, 9)
FAC_v <- c(42, 6)
UPL_FACU_v <- c(26, 93)
Forb_v <- c(51, 51)
Graminoid_v <- c(138, 21)
Woody_v <- c(46, 36)

DF_Spider_2v <- data.frame(Nativity_v, Short_lived_v, Long_lived_v, OBL_FACW_v, FAC_v, UPL_FACU_v, Forb_v, Graminoid_v, Woody_v)
...
tv$'1'<-1
tv$'2'<-0
tv$Native<-as.numeric(tv$Native)/235
tv$Introduced<-as.numeric(tv$Introduced)/108
...
```{r graph native/intro by bank location_NOPE}
