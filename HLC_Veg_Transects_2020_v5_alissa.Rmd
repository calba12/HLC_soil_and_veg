...
vegdf_bout1 <- subset(vegdf,sample.bout =="1") #subset only bout 1; this is the data frame to use! or use vegdf and filter for sample bout 1 (which is what most of the code does)
...
Spp_prop_tbl_veg <- Spp_prop_tbl_veg %>% 
  mutate(TotalPropVeg = Freq2/sum(Spp_prop_tbl_veg$Freq2))

view(Spp_prop_tbl_veg)
...
#means across canal (not by bank) for stats ###HERE
df_prop_hit_type_means2 <- df.prop_hit_type %>%
  group_by(Surface.Hit) %>%
  dplyr::summarise(Mean = mean(prop_cover_type), AvgSD=sd(prop_cover_type))

View(df_prop_hit_type_means2)


#means banks for stats
df_prop_hit_type_means3 <- df.prop_hit_type %>%
  group_by(Bank_Location, Surface.Hit) %>%
  dplyr::summarise(Mean = mean(prop_cover_type))

View(df_prop_hit_type_means3)
...
#For STATS -- Across canal
df_WIS_means_veg3 <- df.props_WIS_veg %>%
  group_by(WIS_GP) %>%
  dplyr::summarise(Mean = mean(prop), AvgSD=sd(prop))

View(df_WIS_means_veg3)

#For STATS -- Bank location
df_WIS_means_veg4 <- df.props_WIS_veg %>%
  group_by(Bank_Location, WIS_GP) %>%
  dplyr::summarise(Mean = mean(prop), AvgSD=sd(prop))

View(df_WIS_means_veg4)
...
#####
#For STATS -- Across canal
df_nativ_means_veg3 <- df.props_Nativity_veg %>%
  group_by(Nativity) %>%
  dplyr::summarise(Mean = mean(prop), AvgSD=sd(prop))

View(df_nativ_means_veg3)

#For STATS -- Bank location
df_nativ_means_veg4 <- df.props_Nativity_veg %>%
  group_by(Bank_Location, Nativity) %>%
  dplyr::summarise(Mean = mean(prop), AvgSD=sd(prop))

View(df_nativ_means_veg4)
####
...
#For STATS -- Across canal
df_FG_means_veg3 <- df.props_FG_veg %>%
  group_by(FunctionalGroup2) %>%
  dplyr::summarise(Mean = mean(propFG), AvgSD=sd(propFG))

View(df_FG_means_veg3)

#For STATS -- Bank location
df_FG_means_veg4 <- df.props_FG_veg %>%
  group_by(Bank_Location, FunctionalGroup2) %>%
  dplyr::summarise(Mean = mean(propFG), AvgSD=sd(propFG))

View(df_FG_means_veg4)
...
df.props_RankAbund_veg <- vegdf %>% ### WHYYYYYYY is it including sample bout 1?????
  group_by(sample.bout, baseline, baseline.rep, Transect, Hit.Number, Bank_Location, VegHit_SoilSurfaceHit) %>%
  filter(sample.bout =="1") %>%
  subset(VegHit_SoilSurfaceHit!="bare soil" & VegHit_SoilSurfaceHit!="downed woody debris" & VegHit_SoilSurfaceHit!="litter" & VegHit_SoilSurfaceHit!="rock" & VegHit_SoilSurfaceHit!="water" & VegHit_SoilSurfaceHit!="NULL") %>%
  drop_na(VegHit_SoilSurfaceHit) %>%  ## drops rows with missing values NA in either Species.Hit 
  summarise(SpeciesCount = n()) %>%  ## from the group_by groupings, count the number 
  group_by(sample.bout, baseline, Transect, Bank_Location, VegHit_SoilSurfaceHit) %>%  
  summarise(SpeciesCount = sum(SpeciesCount)) %>% ## groups by baseline and Bank_location to sum species.Hit
  mutate(prop = SpeciesCount/sum(SpeciesCount)) %>% ## mutate the df by adding prop that is each #sp/total
  complete(VegHit_SoilSurfaceHit, fill = list(SpeciesCount = 0, prop = 0)) %>% ## keep all levels of factor and replace NA with zeros for both Species.Hit and prop
  subset(VegHit_SoilSurfaceHit!="bare soil" & VegHit_SoilSurfaceHit!="downed woody debris"
         & VegHit_SoilSurfaceHit!="litter" & VegHit_SoilSurfaceHit!="rock" & VegHit_SoilSurfaceHit!="water" & VegHit_SoilSurfaceHit!="NULL") %>%
  filter(sample.bout =="1") #remove after complete function so they aren't included in graphs (same as with FG)

## trying to start with ONLY SAMPLE BOUT 1

vegdf_bout1$VegHit_SoilSurfaceHit <- as.factor(vegdf_bout1$VegHit_SoilSurfaceHit)


df.props_RankAbund_veg_bout1 <- vegdf_bout1 %>% ### WHYYYYYYY is it including sample bout 1?????
  group_by(baseline, baseline.rep, Transect, Hit.Number, Bank_Location, VegHit_SoilSurfaceHit) %>%
  subset(VegHit_SoilSurfaceHit!="bare soil" & VegHit_SoilSurfaceHit!="downed woody debris" & VegHit_SoilSurfaceHit!="litter" & VegHit_SoilSurfaceHit!="rock" & VegHit_SoilSurfaceHit!="water" & VegHit_SoilSurfaceHit!="NULL") %>%
  drop_na(VegHit_SoilSurfaceHit) %>%  ## drops rows with missing values NA in either Species.Hit 
  summarise(SpeciesCount = n()) %>%  ## from the group_by groupings, count the number 
...
view(df.props_RankAbund_veg_bout1)


# Check it adds to one with the 5 groups  
sum(df.props_RankAbund_veg$prop[1:57])
    
View(df.props_RankAbund_veg)

#get the means across transect (transected is nested within baseline as replication)
df_RankAbund_means_veg <- df.props_RankAbund_veg_bout1 %>%
...
#get the means again but average bank location as well
df_RankAbund_means_veg2 <- df.props_RankAbund_veg_bout1 %>%
...
# means again but average across all to get most common species #absolute mean for each species
df_RankAbund_means_veg3 <- df.props_RankAbund_veg_bout1 %>%
  group_by(VegHit_SoilSurfaceHit) %>%
  dplyr::summarise(Mean = mean(prop), AvgSD=sd(prop))

view(df_RankAbund_means_veg3)
...
RankAbund_veg1 <- df_RankAbund_means_veg2 %>%
  ...
  mutate(VegHit_SoilSurfaceHit = fct_relevel(VegHit_SoilSurfaceHit, "Bromus inermis", "Phalaris arundinacea", "Elymus repens", "Rhamnus cathartica", "Dactylis glomerata", "Cirsium arvense", "Taraxacum officinale",  "Parthenocissus vitacea", "Convolvulus arvensis", "Symphyotrichum lanceolatum ssp. hesperium")) %>% 
...
RankAbund_veg1 <- df_RankAbund_means_veg2 %>%
  ggplot(aes(x=reorder(factor(VegHit_SoilSurfaceHit),-Mean), y=Mean))+ 
  geom_boxplot()+
    geom_point()+
    labs(x = "Species", y = "Proportion of Cover")+
    stat_summary(fun=mean, geom="point", aes(group = VegHit_SoilSurfaceHit), position = position_dodge(.9), shape=20, size=2.5, color="purple", fill="purple")+
...
RankAbund_veg2 <- df_RankAbund_means_veg2 %>%
  ggplot(aes(x=reorder(factor(VegHit_SoilSurfaceHit),-Mean), y=Mean))+ 
  geom_boxplot()+
  geom_point(data =df_RankAbund_means_veg3, mapping = aes (x=VegHit_SoilSurfaceHit, y =Mean), color="purple")+
  geom_line(data = df_RankAbund_means_veg3, mapping = aes(x = VegHit_SoilSurfaceHit, y = Mean, group=1)) +
  labs(x = "Species", y = "Proportion of Cover")+
    theme_bw(base_size = 10)+
    theme(legend.position = "none", axis.text.x=element_text(angle=45, hjust=1))

print(RankAbund_veg2)



##switching order again and changing species names to ranks
df_RankAbund_means_veg3$rank <- NA
df_RankAbund_means_veg3$rank[order(-df_RankAbund_means_veg3$Mean)] <- 1:nrow(df_RankAbund_means_veg3)
jointdataset <- merge(df_RankAbund_means_veg2, df_RankAbund_means_veg3, by = 'VegHit_SoilSurfaceHit')
jointdataset <- jointdataset %>% 
        rename("Mean" = "Mean.x",
               "baseline_mean" = "Mean.y")

RankAbund_Veg3 <- jointdataset %>%
  ggplot(aes(x=reorder(factor(rank), -baseline_mean), y=Mean))+ 
  geom_boxplot()+
    geom_point(data =df_RankAbund_means_veg3, mapping = aes (x=rank, y =Mean), color="purple") +
    geom_line(data = df_RankAbund_means_veg3, mapping = aes(x = rank, y = Mean, group=1)) +
    labs(x = "Species", y = "Proportion of Cover")+
    theme_bw(base_size = 10)+
    theme(legend.position = "none", axis.text.x=element_text(angle=0, hjust=1))
    
df_RankAbund_means_veg3

print(RankAbund_Veg3)
...
veg_df_lower <- vegdf_bout1 %>%
  subset(Bank_Location=="lower")

df.props_RankAbund_veg_lower <- veg_df_lower %>% 
  group_by(baseline, baseline.rep, Transect, Hit.Number, Bank_Location, VegHit_SoilSurfaceHit) %>%
  subset(VegHit_SoilSurfaceHit!="bare soil" & VegHit_SoilSurfaceHit!="downed woody debris" & VegHit_SoilSurfaceHit!="litter" & VegHit_SoilSurfaceHit!="rock" & VegHit_SoilSurfaceHit!="water" & VegHit_SoilSurfaceHit!="NULL") %>%
  drop_na(VegHit_SoilSurfaceHit) %>%  ## drops rows with missing values NA in either Species.Hit 
  summarise(SpeciesCount = n()) %>%  ## from the group_by groupings, count the number 
...
view(df.props_RankAbund_veg_lower)


# Check it adds to one with the 5 groups  
sum(df.props_RankAbund_veg_lower$prop[1:57])
    
View(df.props_RankAbund_veg_lower)


# means again but average across all to get most common species #absolute mean for each species
df_RankAbund_means_veg3_lower <- df.props_RankAbund_veg_lower %>%
  group_by(VegHit_SoilSurfaceHit) %>%
  dplyr::summarise(Mean = mean(prop), AvgSD=sd(prop))

view(df_RankAbund_means_veg3_lower)

write_xlsx(df_RankAbund_means_veg3_lower, 'VEG_SpeciesProportions_lowerbank2023.xlsx') 




###MIDDLE
veg_df_middle <- vegdf_bout1 %>%
  subset(Bank_Location=="middle")

df.props_RankAbund_veg_middle <- veg_df_middle %>% 
  group_by(baseline, baseline.rep, Transect, Hit.Number, Bank_Location, VegHit_SoilSurfaceHit) %>%
  subset(VegHit_SoilSurfaceHit!="bare soil" & VegHit_SoilSurfaceHit!="downed woody debris" & VegHit_SoilSurfaceHit!="litter" & VegHit_SoilSurfaceHit!="rock" & VegHit_SoilSurfaceHit!="water" & VegHit_SoilSurfaceHit!="NULL") %>%
  drop_na(VegHit_SoilSurfaceHit) %>%  ## drops rows with missing values NA in either Species.Hit 
  summarise(SpeciesCount = n()) %>%  ## from the group_by groupings, count the number 
...
view(df.props_RankAbund_veg_middle)


# Check it adds to one with the 5 groups  
sum(df.props_RankAbund_veg_middle$prop[1:57])
    
View(df.props_RankAbund_veg_middle)


# means again but average across all to get most common species #absolute mean for each species
df_RankAbund_means_veg3_middle <- df.props_RankAbund_veg_middle %>%
  group_by(VegHit_SoilSurfaceHit) %>%
  dplyr::summarise(Mean = mean(prop), AvgSD=sd(prop))

view(df_RankAbund_means_veg3_middle)

write_xlsx(df_RankAbund_means_veg3_middle, 'VEG_SpeciesProportions_middlebank2023.xlsx') 
...
###Upper
veg_df_upper <- vegdf_bout1 %>%
  subset(Bank_Location=="upper")

df.props_RankAbund_veg_upper <- veg_df_upper %>% 
  group_by(baseline, baseline.rep, Transect, Hit.Number, Bank_Location, VegHit_SoilSurfaceHit) %>%
  subset(VegHit_SoilSurfaceHit!="bare soil" & VegHit_SoilSurfaceHit!="downed woody debris" & VegHit_SoilSurfaceHit!="litter" & VegHit_SoilSurfaceHit!="rock" & VegHit_SoilSurfaceHit!="water" & VegHit_SoilSurfaceHit!="NULL") %>%
  drop_na(VegHit_SoilSurfaceHit) %>%  ## drops rows with missing values NA in either Species.Hit 
  summarise(SpeciesCount = n()) %>%  ## from the group_by groupings, count the number 
...
# Check it adds to one with the 5 groups  
sum(df.props_RankAbund_veg_upper$prop[1:57])
    
View(df.props_RankAbund_veg_upper)


# means again but average across all to get most common species #absolute mean for each species
df_RankAbund_means_veg3_upper <- df.props_RankAbund_veg_upper %>%
  group_by(VegHit_SoilSurfaceHit) %>%
  dplyr::summarise(Mean = mean(prop), AvgSD=sd(prop))

view(df_RankAbund_means_veg3_upper)

write_xlsx(df_RankAbund_means_veg3_upper, 'VEG_SpeciesProportions_upperbank2023.xlsx') 
...
#veg
view(df_RankAbund_means_veg3) #table was created under "graph rank abundance curve" and contains average proportion and SD for each baseline 

view(Spp_prop_tbl_veg) #created under "create species tables" has TOTAL proportion and frequency and other info about each species. 

#first I need to combine these two tables
merged_proportions_veg <- merge(Spp_prop_tbl_veg, df_RankAbund_means_veg3, by = 'VegHit_SoilSurfaceHit',all.x =TRUE)
view(merged_proportions_veg)

######now need to merge veg and soil 

#change VegHit_SoilSurfaceHit to Species_Name
merged_proportions_veg <- merged_proportions_veg %>% rename(Species_Name=VegHit_SoilSurfaceHit)

#merge
merged_proportions_VegSeed <- merge(merged_proportions_veg, merged_proportions_seed, by = 'Species_Name',all.x =TRUE)

merged_proportions_VegSeed <- merge(merged_proportions_veg, merged_proportions_seed, by.x = c("Species_Name", "Longevity", "Habit", "WIS_GP", "Nativity"), by.y = c("Species_Name", "Longevity_1", "Habit", "WIS", "Nativity"), all.x =TRUE, all.y = TRUE)
#x=veg
#y=seed
view(merged_proportions_VegSeed)

write_xlsx(merged_proportions_VegSeed, 'merged_seedveg_AvgandTotalProps_02092023.xlsx')
