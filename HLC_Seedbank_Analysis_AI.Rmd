...
df_Spp <- distinct(seedbankdf, Species_Name, .keep_all = TRUE) %>%
  ...
  select(Species_Name, Bank_Location, Longevity_1, Habit, Functional_Group_2, Nativity, WIS, C_Value)

View(df_Spp)

#Export to Excel
#write_xlsx(df_Spp, 'SB_SpeciesList_V2_AI_08022022.xlsx') 


Spp_tbl <- seedbankdf %>%
  select(Species_Name, Longevity_1, Habit, Nativity, WIS, C_Value)%>% 
  group_by(Species_Name, Longevity_1, Habit, Nativity, WIS, C_Value) %>%
  summarise(Freq2 = n())

Spp_prop_tbl <- Spp_tbl %>% 
  mutate(prop = Freq2/sum(Spp_tbl$Freq2))

view(Spp_prop_tbl)
sum(Spp_tbl$Freq2)
#write_xlsx(Spp_prop_tbl, 'SB_SppList_Prop_AI08052022.xlsx')

#merge with veg prop data

veg_prop_tbl <- read.csv("./Veg_SppList_Prop3_AI08072022.csv")

merged_spp_prop <- merge(veg_prop_tbl, Spp_prop_tbl, by.x = c("VegHit_SoilSurfaceHit", "Longevity", "Habit", "WIS", "Nativity", "Cvalue" ), by.y = c("Species_Name", "Longevity_1", "Habit", "WIS", "Nativity", "C_Value"), all.x =TRUE, all.y = TRUE)

view(merged_spp_prop)

write_xlsx(merged_spp_prop, 'merged_SppList_Prop_AI08072022.xlsx')
