...
df_sb <- seedbankdf %>% 
  ...
  filter(Species_Name!="Unknown" & Species_Name!="Unknown forb"
...
SB_Spp_tbl <- seedbankdf %>%
  ...
  filter(Species_Name !="Unknown Poaceae") %>% 
...
SB_Spp_tbl <- SB_Spp_tbl %>% #total proportions (not averaged)
  mutate(totalprop = Freq2/sum(SB_Spp_tbl$Freq2))
...
SB_families <- seedbankdf %>% 
  group_by(Family, Species_Name) %>%
  summarise(Species.Name = n()) 

view(SB_families)

write_xlsx(SB_families, 'sb_fam_counts_2023.xlsx')
...
##########
#Means for stats
df_WIS_means3 <- df.props_WIS_seed %>%
  group_by(WIS) %>%
  dplyr::summarise(Mean = mean(prop), AvgSD=sd(prop))
View(df_WIS_means3)

df_WIS_means4 <- df.props_WIS_seed %>%
  group_by(Bank_Location, WIS) %>%
  dplyr::summarise(Mean = mean(prop))
View(df_WIS_means4)

df_WIS_means5 <- df.props_WIS_seed %>%
  group_by(Baseline, WIS) %>%
  dplyr::summarise(Mean = mean(prop),  AvgSD=sd(prop))
View(df_WIS_means5)
...
##########
#Means for stats
df_FGroup_means_sb3 <- df.props_FunctGroup_seed %>%
  group_by(FunctionalGroup) %>%
  dplyr::summarise(Mean = mean(prop), AvgSD=sd(prop))
View(df_FGroup_means_sb3)

df_FGroup_means_sb4 <- df.props_FunctGroup_seed %>%
  group_by(Bank_Location, FunctionalGroup) %>%
  dplyr::summarise(Mean = mean(prop))
View(df_FGroup_means_sb4)
...
##########
#Means for stats
df_nativity_means2 <- df.props_nativity_sb %>%
  group_by(Nativity) %>%
  dplyr::summarise(Mean = mean(prop), AvgSD=sd(prop))
View(df_nativity_means2)

df_nativity_means3 <- df.props_nativity_sb %>%
  group_by(Bank_Location, Nativity) %>%
  dplyr::summarise(Mean = mean(prop))
View(df_nativity_means3)
...
df.props_RankAbund_seed <- seedbankdf %>%
 ...
 filter(Species_Name !="Unknown") %>%
 filter(Species_Name !="Unknown forb") %>%
  group_by(Baseline, Transect_ID, Bank_Location, Species_Name) %>%
  drop_na(Species_Name) %>%  ## drops rows with missing values NA in either SPECIFIC_EPITHET_FINAL or WIS
  summarise(SpeciesCount = n()) %>%  ## from the group_by groupings, count the number 
  group_by(Baseline, Transect_ID, Bank_Location, Species_Name) %>% #added transect so I can get mean
  summarise(SpeciesCount = sum(SpeciesCount)) %>%  ## groups by baseline and Bank_location to sum SPECIFIC_EPITHET_FINAL
  mutate(prop = SpeciesCount/sum(SpeciesCount)) %>% ## mutate the df by adding prop that is each #sp/total
  complete(Species_Name, fill = list(SpeciesCount = 0, prop = 0)) %>% ## keep all levels of factor and replace NA with zeros for both   SPECIFIC_EPITHET_FINAL and prop
  filter(Species_Name !="Unknown Poaceae") %>% #removing these groups from the graph (NAs were already dropped)
 filter(Species_Name !="Unknown graminoid") %>%
 filter(Species_Name !="Unknown") %>%
 filter(Species_Name !="Unknown forb")
...
#banklocation
df_RankAbund_means_seed_BL <- df.props_RankAbund_seed %>%
  group_by(Bank_Location, Species_Name) %>%
  dplyr::summarise(Mean = mean(prop), AvgSD=sd(prop))

view(df_RankAbund_means_seed_BL)
#get the means across transect (transected is nested within baseline as replication)// not this one
...
#get the means again but average bank location as well /// this one is used for creating graph
df_RankAbund_means_seed2 <- df.props_RankAbund_seed %>%
  group_by(Baseline, Species_Name) %>%
  dplyr::summarise(AvgProp = mean(prop))

View(df_RankAbund_means_seed2)


# means again but average across all to get most common species #absolute mean
df_RankAbund_means_seed3 <- df.props_RankAbund_seed %>%
  group_by(Species_Name) %>%
  dplyr::summarise(AvgProp = mean(prop), AvgSD=sd(prop))

View(df_RankAbund_means_seed3)

Common_seed_species <- df_RankAbund_means_seed3 %>% #ignore
  filter(Mean > 0.0199) %>%
  arrange(Mean)
...
df_RankAbund_means_seed3

print(RankAbund_seed2)

##switching order again and changing species names to ranks
df_RankAbund_means_seed3$rank <- NA
df_RankAbund_means_seed3$rank[order(-df_RankAbund_means_seed3$Mean)] <- 1:nrow(df_RankAbund_means_seed3)
jointdataset <- merge(df_RankAbund_means_seed2, df_RankAbund_means_seed3, by = 'Species_Name')
jointdataset <- jointdataset %>% 
        rename("Mean" = "Mean.x",
               "baseline_mean" = "Mean.y")

RankAbund_seed2 <- jointdataset %>%
  ggplot(aes(x=reorder(factor(rank), -baseline_mean), y=Mean))+ #not reordered perfectly, not sure how to fix
  geom_boxplot()+
    geom_point(data =df_RankAbund_means_seed3, mapping = aes (x=rank, y =Mean), color="purple") +
    geom_line(data = df_RankAbund_means_seed3, mapping = aes(x = rank, y = Mean, group=1)) +
    labs(x = "Species", y = "Proportion of Seed Bank")+
    theme_bw(base_size = 10)+
...
```{r Create table with total proportion and average proportion}

# means again but average across all to get most common species #absolute mean
df_RankAbund_means_seed4 <- df.props_RankAbund_seed %>%
  group_by(Family, Species_Name) %>%
  dplyr::summarise(AvgProp = mean(prop), AvgSD=sd(prop))

View(df_RankAbund_means_seed4)


view(df_RankAbund_means_seed4) #table was created under "graph rank abundance curve" and contains average proportion and SD for each baseline #changed name from seed3 > seed4

view(SB_Spp_tbl) #created under "create species tables" has TOTAL proportion and frequency and other info about each species. 

#first I need to combine these two tables
merged_proportions_seed <- merge(SB_Spp_tbl, df_RankAbund_means_seed3, by = 'Species_Name',all.x =TRUE)
view(merged_proportions_seed)

#then I need to merge this with the vegetation data? 

```

```{r species proportions based on bank location, averaged }

seedbankdf$Species <- as.factor(seedbankdf$Species)
seedbankdf$Species_Name <- as.factor(seedbankdf$Species_Name)
seedbankdf$WIS <- as.factor(seedbankdf$WIS)
seedbankdf$Functional_Group_1 <- as.factor(seedbankdf$Functional_Group_1)
seedbankdf$Nativity <- as.factor(seedbankdf$Nativity)



seed_df_lower <- seedbankdf %>%
  subset(Bank_Location=="Lower")

#LOWER
df.props_RankAbund_seed_lower <- seed_df_lower %>%
  filter(Species_Name !="Unknown Poaceae") %>% #removing these groups from the graph (NAs were already dropped)
 filter(Species_Name !="Unknown graminoid") %>%
 filter(Species_Name !="Unknown") %>%
 filter(Species_Name !="Unknown forb") %>%
...
# Check it adds to one with the 5 groups  
sum(df.props_RankAbund_seed_lower$prop[1:50])
    
View(df.props_RankAbund_seed_lower)


# means again but average across all to get most common species #absolute mean
df_RankAbund_means_seed3_lower <- df.props_RankAbund_seed_lower %>%
  group_by(Species_Name) %>%
  dplyr::summarise(AvgProp = mean(prop), AvgSD=sd(prop))

View(df_RankAbund_means_seed3_lower)

write_xlsx(df_RankAbund_means_seed3_lower, 'SB_SpeciesProportions_lowerbank2023.xlsx') 

#MIDDLE
seed_df_middle <- seedbankdf %>%
  subset(Bank_Location=="Middle")

df.props_RankAbund_seed_middle <- seed_df_middle %>%
  filter(Species_Name !="Unknown Poaceae") %>% #removing these groups from the graph (NAs were already dropped)
 filter(Species_Name !="Unknown graminoid") %>%
 filter(Species_Name !="Unknown") %>%
 filter(Species_Name !="Unknown forb") %>%
...
# Check it adds to one with the 5 groups  
sum(df.props_RankAbund_seed_middle$prop[1:50])
    
View(df.props_RankAbund_seed_middle)


# means again but average across all to get most common species #absolute mean
df_RankAbund_means_seed3_middle <- df.props_RankAbund_seed_middle %>%
  group_by(Species_Name) %>%
  dplyr::summarise(AvgProp = mean(prop), AvgSD=sd(prop))

View(df_RankAbund_means_seed3_middle)


write_xlsx(df_RankAbund_means_seed3_middle, 'SB_SpeciesProportions_middlebank2023.xlsx') 




#UPPER
seed_df_upper <- seedbankdf %>%
  subset(Bank_Location=="Upper")

df.props_RankAbund_seed_upper <- seed_df_upper %>%
  filter(Species_Name !="Unknown Poaceae") %>% #removing these groups from the graph (NAs were already dropped)
 filter(Species_Name !="Unknown graminoid") %>%
 filter(Species_Name !="Unknown") %>%
 filter(Species_Name !="Unknown forb") %>%
...
# Check it adds to one with the 5 groups  
sum(df.props_RankAbund_seed_upper$prop[1:50])
    
View(df.props_RankAbund_seed_upper)


# means again but average across all to get most common species #absolute mean
df_RankAbund_means_seed3_upper <- df.props_RankAbund_seed_upper %>%
  group_by(Species_Name) %>%
  dplyr::summarise(AvgProp = mean(prop), AvgSD=sd(prop))

View(df_RankAbund_means_seed3_upper)

write_xlsx(df_RankAbund_means_seed3_upper, 'SB_SpeciesProportions_upperbank2023.xlsx') 
