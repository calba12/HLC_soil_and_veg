...
df.props_WIS_seed$prop2 <- df.props_WIS_seed$prop *100

glmer_pois_null <- glmer(prop2 ~ 1 + (1|Baseline) + (1|Transect_ID:Baseline), data = df.props_WIS_seed, family = poisson(link = log)) #intercept only, just models variation across baselines and transects, no accounting for variation explained by WIS
glmer_pois1 <- glmer(prop ~ WIS + (1|Baseline) + (1|Transect_ID:Baseline), data = df.props_WIS_seed, family = poisson(link = log)) #add in variation explained by WIS
glmer_pois2 <- glmer(prop ~ WIS + Bank_Location + (1|Baseline) + (1|Transect_ID:Baseline), data = df.props_WIS_seed, family = poisson(link = log)) #add in variation explained by bank location
glmer_pois3 <- glmer(prop ~ WIS + Bank_Location + WIS*Bank_Location + (1|Baseline) + (1|Transect_ID:Baseline), data = df.props_WIS_seed, family = poisson(link = log)) #add in interaction term
glmer_pois4 <- glmer(prop ~ WIS + Bank_Location + WIS*Bank_Location + (1|Baseline), data = df.props_WIS_seed, family = poisson(link = log)) #Remove random effect of transect within baseline
...
summary(glmer_pois1) #Get model output
summary(glmer_pois2) #Get model output
summary(glmer_pois3) #Get model output
summary(glmer_pois4) #Get model output
...
proportion_WIS_pois <- df.props_WIS_seed$prop #Histogram showing lots of zeroes, positive skew 
hist(proportion_WIS_pois)
qqnorm(df.props_WIS_seed$prop) #use to visualize whether raw data are normally distributed
qqnorm(resid(glmer_pois4))     #Use to visualize whether model residuals vs predicted are normally distributed
...
#Diagnostics
plot_residuals(glmer_pois4)
plot_model(glmer_pois4, type = "diag")
sjPlot::plot_model(glmer_pois4, type = "est", show.values = T)

#creates table; information from the ANOVA
sjPlot::tab_model(glmer_pois4, df.method = "satterthwaite", title = "Proportion WIS in Vegetation", dv.labels = "Proportion Cover")

#post hoc: tukey/ familywise error rate
emmeans(glmer_pois4, df.method = "satterthwaite", pairwise ~ WIS | Bank_Location, adjust = "tukey")
emmip(glmer_pois4, WIS ~ Bank_Location, CIs = TRUE)
emmip(glmer_pois4, WIS ~ Bank_Location)
...
lmer_gaus_null <- glmer(prop ~ 1 + (1|Baseline) + (1|Transect:Baseline), data = df.props_nativity_sb) #intercept only, just models variation across baselines and transects, no accounting for variation explained by nativity
lmer_gaus_native_1 <- glmer(prop ~ Nativity + (1|Baseline) + (1|Transect:Baseline), data = df.props_nativity_sb) #add in variation explained by nativity
lmer_gaus_native_2 <- glmer(prop ~ Nativity + Bank_Location + (1|Baseline) + (1|Transect:Baseline), data = df.props_nativity_sb) #add in variation explained by bank location
lmer_gaus_native_3 <- glmer(prop ~ Nativity + Bank_Location + Nativity*Bank_Location + (1|Transect:Baseline) + (1|Transect:baseline), data = df.props_nativity_sb) #add in interaction term
lmer_gaus_native_4 <- glmer(prop ~ Nativity + Bank_Location + Nativity*Bank_Location + (1|Transect:Baseline), data = df.props_nativity_sb) #Remove random effect of transect within baseline
