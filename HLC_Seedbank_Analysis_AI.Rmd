...
WIS <- df_WIS_means %>% 
  ggplot(aes(x=factor(WIS, levels = c('UPL', 'FACU', 'FAC', 'FACW', 'OBL', 'Juncus sp.')), y=Mean, fill=WIS))+ 
  geom_boxplot()+
    geom_point()+
...
df.props_FunctGroup_seed <- seedbank %>%
  ...
  summarise(SPECIFIC_EPITHET_FINAL = n()) %>%  ## from the group_by groupings, count the number 
  filter(Functional_Group_2 != "Long-lived Woody") %>%
...
FGroup2 <- df_FGroup_means %>% 
  ...
  geom_boxplot()+
    geom_point()+
    labs(x = "FG", y = "Proportion of Seed Bank")+
...
FGroup2 + facet_wrap(~Bank_Location)
