...
#write_xlsx(Spp_prop_tbl, 'AG_SppList_proportions.xlsx')
...
prop_species_canal_FirstBout <- veg_only_transect_FirstBout %>%
  group_by(Species.Hit) %>%
  summarise(Freq = sum(Species)) %>%
  mutate(prop = Freq/sum(838)) %>% #changed to 838 based on frequency count
  filter(prop > 0.0099) %>%
  arrange(desc(prop))
...
sum(prop_ground$Freq)
                ...
                #Only Bout 1
ground_only_transect_bout1 <- vegdf_bout1 %>%
  group_by(sample.bout, Hit.Number, Bank_Location, baseline, Transect, Num_transect_points, Transect.Point.Soil.Surface) %>%
  summarise(Ground = n()) %>%
  drop_na()

head(ground_only_transect_bout1)
#View(ground_only_transect_bout1)
...
prop_ground_bout1 <- ground_only_transect_bout1 %>%
  group_by(Transect.Point.Soil.Surface) %>%
  summarise(Freq = sum(Ground)) %>%
  mutate(prop = Freq/2080) #changed to 2080 to reflect Bout1 
  
View(prop_ground_bout1)

sum(prop_ground_bout1$Freq)
...
```{r graph WIS by baseline, from pivot tbl - don't run}
#This chunk is original code that used data imported from an excel pivot table, don't run
...
df.props_WIS_veg <- vegdf %>%
  ...
  filter(sample.bout =="1") %>% #only uses bout1
  ...
  group_by(baseline, Transect, Bank_Location, WIS_GP) %>%  #added transect so I can get mean (AI)
...
#Took CA code and adjusted
WIS_baseline_long2 <- df.props_WIS_veg_exp #renamed to make changing code easier
...
df_WIS_baseline_means2 %>% 
  mutate(WIS_GP = fct_relevel(WIS_GP, "UPL", "FACU", "FAC", "FACW", "OBL")) %>%
  ggplot(aes(x=WIS_GP, y=Mean)) +
  geom_boxplot()+
  geom_point()+
  labs(title = "Aboveground WIS proportion means")+
  xlab("Wetland Indicator Status (WIS)")+
...
WIS_bankLoc <- read.csv("./WIS_byBankLoc_Upload_WMVC.csv")
str(WIS_bankLoc)
View(WIS_bankLoc)
...
```{r graph WIS by bank location --> AI }
...
#WIS_baseline_long2 should work in place of WIS_bankLoc_long, although it doesn't include transects
#I'm going to change the code so that it works for WIS_baseline_long2
...
p <- WIS_baseline_long2 %>%
  mutate(WIS_GP = fct_relevel(WIS_GP, "UPL", "FACU", "FAC", "FACW", "OBL")) %>%
  ggplot(aes(x=WIS_GP, y=prop))+
  geom_boxplot()+
  geom_point()+
  labs(x = "Wetland Indicator Status", y = "Proportion")+
  theme_bw(base_size = 16)+
...
#df_WIS_baseline_means2 already has mean across transects (from above AI code chunk)
...
View(df_WIS_baseline_means)
#AI -> I'm noticing that df_WIS_baseline_means(From pivot tbls) does not match df_WIS_baseline_means2 (from Michelle's code), why? 
#going to proceed with michelle's code. 

#Now create new boxplot with baseline means

p <- df_WIS_baseline_means2 %>%
  mutate(WIS_GP = fct_relevel(WIS_GP, "UPL", "FACU", "FAC", "FACW", "OBL")) %>%
  ggplot(aes(x=WIS_GP, y=Mean)) +
  geom_boxplot()+
  geom_point()+
  labs(x = "Wetland Indicator Status", y = "Proportion Vegetation Cover")+
  stat_summary(fun=mean, geom="point", aes(group = WIS_GP), position = position_dodge(.9), shape=20, size=2.5, color="red", fill="red")+
  theme_bw(base_size = 16)+
  theme(axis.text.x=element_text(angle=90, hjust=1))
...
###8/20/22 AI this is where I left off
